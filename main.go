...
// redactURIHost returns only host information for safe logging.
func redactURIHost(uri string) string {
	u, err := url.Parse(uri)
	if err != nil {
		return "[redacted]"
	}
...
func buildExporter(opts GlobalFlags, uri string, log *logrus.Logger) *exporter.Exporter {
	...
	log.Debugf("Connecting to MongoDB host: %s", redactURIHost(uri))
